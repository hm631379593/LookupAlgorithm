...
*/
template <class T>
bool BTree::BTree_insert(T value) {
...
*/
template <class T>
void Btree:: SplitBlock(BTnode<T>* node_parent, int child_index, BTnode<T>* node_child)
{
...
*/
template <class T>
void Btree::Notfull_insert(BTnode<t>* node, T value) {
...
*/
template <class T>
BTnode<T>* Btree:: BTree_find(BTnode<T>* node, T value) {
...
*/
template <class T>
void Btree::printpoint(BTnode<T>* node, int count)
{
	if (node != NULL)//判断元素是否为空
	{
		...
		for (i = 0; i < node->keyNum; i++)
		{
			...
			cout << "|" << node->keyvalue[i] << "|" << endl; if = "" node -= "" > isleaf)//子节点数比关键字数多一个
		}
	}
	else{
		printpoint(node->pchild[i], count - 3);
...
*/
template <class T>
bool Btree:: BTree_delete(T value) {
	if (!contain(value))
	{
		return false;
	}
...
*/
template <class T>
void Btree:: MergeBlock(BTnode<T>* node_parent, int child_index) {
...
*/
template <class T>
void Btree::BTree_deletebalance(BTnode<T>* node, T value)
{
	int i;
	...
	{
		//在(<<value<<)的区间找 t="">* node_child = node->pchild[i];//这里i = node->keyNum
